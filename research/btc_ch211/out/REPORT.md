...
Модель V_sw(SDO 211Å) — прогнозная (~4 дня форы), и она реально работает по своему назначению: с фактической скоростью ветра (ACE) r = **+0.50** за год — это та самая сильная корреляция, которая видна на сайте SINP. Вопрос теста: конвертируется ли она в сделки по биткоину.

Правила: LONG после подтверждённого дна, SHORT после подтверждённого верха; вход от −2 дней до экстремума (фора прогноза минус 2 дня на разворот) до +3; удержание 2–10 дней; каждая клетка против тех же входов в случайные даты.

| правило | вход | удержание | сделок | средняя | случайные даты | избыток | p |
|---|---|---|---|---|---|---|---|
| SHORT с верха | -2д | 2д | 33 | +1.12% | +0.36% | +0.77 п.п. | 0.083 |
| LONG со дна | +2д | 10д | 30 | -0.33% | -1.98% | +1.65 п.п. | 0.101 |
| SHORT с верха | -2д | 7д | 33 | +2.32% | +1.35% | +0.97 п.п. | 0.164 |
| SHORT с верха | +1д | 3д | 33 | +1.14% | +0.55% | +0.58 п.п. | 0.203 |
| SHORT с верха | -2д | 5д | 33 | +1.65% | +0.96% | +0.69 п.п. | 0.209 |
| LONG со дна | +3д | 10д | 30 | -0.95% | -2.00% | +1.05 п.п. | 0.221 |

Клеток в сетке: 60, с p<0.05: 0 (шум при таком переборе даёт ~3.0). Лучшая клетка после поправки на перебор: p≈1.00.

Прямые корреляции V_sw(211Å) × BTC по году (та же батарея, что для площади CH):

| разрез | лучший лаг | r | p(скан) |
|---|---|---|---|
| уровни | +3 дн | +0.09 | 0.55 |
| приращения | -10 дн | +0.15 | 0.04 |
...
Вторая выгрузка SINP: скорость/плотность/температура (факт, ACE) и модельные скорость/давление/температура по 193Å — 12 сканов той же батареей.

| серия | разрез | лучший лаг | r | p(скан) | после детренда |
|---|---|---|---|---|---|
| скорость СВ, модель 193Å | уровни | +2 дн | +0.16 | 0.005 | r=+0.17, p=0.28 |
| температура СВ, модель 193Å | уровни | +2 дн | +0.17 | 0.005 | r=+0.16, p=0.41 |
| температура ионов, факт | приращения | +3 дн | +0.18 | 0.041 | — |
| плотность протонов, факт | уровни | -9 дн | +0.18 | 0.059 | r=+0.11, p=0.34 |
| скорость СВ, факт (Bulk) | приращения | +3 дн | +0.15 | 0.101 | — |
| температура СВ, модель 193Å | приращения | -10 дн | +0.15 | 0.155 | — |
| плотность протонов, факт | приращения | -9 дн | +0.15 | 0.163 | — |
| давление СВ, модель 193Å | приращения | +2 дн | -0.15 | 0.195 | — |
| скорость СВ, модель 193Å | приращения | -10 дн | +0.14 | 0.200 | — |
| давление СВ, модель 193Å | уровни | +2 дн | -0.08 | 0.321 | r=-0.21, p=0.04 |
| скорость СВ, факт (Bulk) | уровни | -11 дн | +0.11 | 0.716 | r=+0.18, p=0.46 |
| температура ионов, факт | уровни | -11 дн | +0.12 | 0.724 | r=+0.20, p=0.11 |

Модельные серии по 193Å (скорость/давление/температура) выводятся из одной и той же площади дыр — это не независимые проверки, а варианты сигнала из раздела 3.


Сканов: 12; с p<0.05: 3 (шум при таком переборе даёт ~0.6); порог Бонферрони 0.0042 — **ни одна серия его не проходит**.
...
**Торговый тест от горбов ветра.** Сильная корреляция здесь действительно есть — модель скорости ветра против фактической: r=+0.50 за год. Это физика, она работает. Мы проверили, конвертируется ли она в сделки: 60 вариантов входа/удержания от доньев и вершин горбов, каждый против случайных дат. Лучший вариант (шорт с верха горба, вход -2д, удержание 2д) даёт +1.1% за сделку против +0.4% у случайных входов; p=0.08 до поправки и ≈1.00 после поправки на перебор 60 клеток — преимущество не отличимо от везения.

**Скрининг всей плазмы ветра.** Мы проверили против биткоина ещё шесть параметров солнечного ветра (скорость, плотность и температуру — фактические и модельные): 12 проверок, из них «подозрительных» (p<0.05) — 3, а чистый шум при таком переборе даёт в среднем 0.6. Семейный порог (поправку на перебор) не прошла ни одна серия: солнечный ветер целиком — не только площадь дыр — курс не объясняет.
