...
# (хронология пожаров). Должен покрывать все ключи FIRE_DATES.
WAREHOUSES = ["Электросталь", "Котовск", "Краснодар", "Невинномысск",
              "Шушары", "Новосаратовка", "Пенза", "Новосемейкино", "Тула"]

# дата пожара → срез остатков берём за предыдущий день
FIRE_DATES = {"Краснодар": "2026-07-22", "Невинномысск": "2026-07-22",
              "Шушары": "2026-07-24", "Новосаратовка": "2026-07-24",
              "Электросталь": "2026-07-18", "Котовск": "2026-07-18",
              "Пенза": "2026-07-30",
              "Новосемейкино": "2026-08-02",
              "Тула": "2026-08-05"}
...
_NOTES = {
    ...
    "Новосемейкино": "Пожар 02.08.2026 (Самарский узел).",
    "Новосаратовка": "Пожар 24.07.2026 (Ленобласть, вместе с Шушарами).",
    "Пенза": "Пожар 30.07.2026 (62 тыс м², включая площадку СГТ).",
    "Тула": "Пожар 05.08.2026.",
}
...
async def fetch_from_storage(warehouse: str, snap_date: str = "") -> dict:
    ...
    if len(match) > 1:
        # несколько площадок одного узла («Пенза» + «Пенза СГТ») — суммируем;
        # непохожие имена суммировать молча нельзя, в претензии нужен адрес
        if not all(m.lower().startswith(want) for m in match):
