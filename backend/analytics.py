...
Works with both mock data (rich finance fields) and the real WB sales
endpoint (only totalPrice/discountPercent/priceWithDisc/forPay) — missing
finance fields are estimated with the coefficients below.
"""
from __future__ import annotations
from collections import defaultdict
from typing import Any

# ─── estimation coefficients (used when WB API doesn't return the field) ─────
COMMISSION_RATE = 0.15   # вознаграждение WB
STORAGE_RATE    = 0.015  # хранение
COST_RATE       = 0.30   # себестоимость от полной цены
AD_RATE         = 0.06   # ДРР (реклама от реализации)
TAX_RATE        = 0.07   # налог УСН от продаж после СПП
ACCEPTANCE_PER  = 1.5    # платная приёмка за единицу
OTHER_RATE      = 0.01   # прочие удержания от реализации
...
# ─── per-item helpers (with fallbacks) ───────────────────────────────────────

def _gross(r: dict) -> float:
    """Продажи после СПП for one sale item."""
...
def finance_aggregate(sales: list[dict], orders: list[dict]) -> dict[str, float]:
    ...
    return {
        ...
        "profit_before_opex": round(profit_before_opex),
...
def finance_cards(cur: dict, prev: dict) -> list[dict]:
    """16 cards (4x4). invert=True → growth is bad (expense)."""
    def card(title, icon, key, unit="₽", secondary="", invert=False):
        c = {
            "title": title, "icon": icon, "unit": unit,
...
def revenue_structure(agg: dict) -> list[dict]:
    """Horizontal bar chart data (Selena-style waterfall)."""
    return [
...
def _abc_map(items: list[tuple[int, float]]) -> dict[int, str]:
    """items: [(nmId, metric)]. Returns nmId -> A/B/C by cumulative 80/95."""
...
def products_table(sales: list[dict], orders: list[dict], days: int) -> list[dict]:
    ...
    for nm, a in skus.items():
        ...
        tax = a["sales_after_spp"] * TAX_RATE
        profits[nm] = a["realization"] - a["cost"] - ad - tax
    ...
    for nm, a in skus.items():
        ...
        rows.append({
            ...
            "roi": round(roi, 1),
            "drr": round(drr, 1),
            "buyout": round(buyout, 1),
            "orders": oq,
            "sold": a["sold"],
            "returns": returns.get(nm, 0),
            "abc_rev": abc_rev.get(nm, "C"),
            "abc_profit": abc_profit.get(nm, "C"),
        })
...
def warehouses(sales: list[dict], orders: list[dict], stocks: list[dict], days: int) -> list[dict]:
    ...
    for wh in sorted(set(stock_wh) | set(sold_wh)):
        ...
        # synthetic stock trend sparkline (declining by per_day)
...
def supplies(sales: list[dict], stocks: list[dict], days: int) -> list[dict]:
    ...
    order = {"urgent": 0, "planned": 1, "ok": 2}
    rows.sort(key=lambda r: (order[r["priority"]], r["coverage_days"]))
    return rows


# ─── legacy turnover / reorder (kept for compatibility) ──────────────────────

def abc_by_turnover(sales: list[dict], stocks: list[dict], days: int) -> list[dict]:
    rows = supplies(sales, stocks, days)
    n = len(rows) or 1
    rows_sorted = sorted(rows, key=lambda r: r["coverage_days"])
