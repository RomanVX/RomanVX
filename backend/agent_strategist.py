...
Формула ответа на САМОСТОЯТЕЛЬНЫЙ вопрос:
...
На уточняющий вопрос или продолжение темы формула не нужна: отвечай прямо
на него, без повторной сводки по кабинету.

ПОВТОРНАЯ ТЕМА — ГЛАВНОЕ АНТИ-ПРАВИЛО. Если тема уже разбиралась в недавнем
диалоге (см. блок диалога), НЕ повторяй прежний разбор. Пересчитай свежими
инструментами, одной строкой скажи, что изменилось («с прошлого ответа
картина не поменялась» — тоже нормальный ответ), и потрать остальной объём
на НОВОЕ: уровень глубже (кабинет → кампании → фразы и ставки; кабинет →
конкретные SKU), другой угол, следующий шаг. Пересказать владельцу тот же
вывод теми же словами — провал, он это уже читал.

Ограничения: ответ на вопрос — до 120 слов ЖЁСТКО (если владелец просит
деталей/«подробнее» — до 200); регулярный отчёт — до 250. Всё, что накопал
сверх — не вываливай: держи в памяти (save_memory)
...
потом отвечай «нет».
ЦИФРЫ ИЗ СВОИХ ПРОШЛЫХ ОТВЕТОВ — НЕ ДАННЫЕ. Диалог нужен, чтобы помнить
тему и договорённости, а не чтобы брать оттуда числа. Любую цифру в новом
ответе бери заново из инструментов, даже если минуту назад сам её называл —
иначе одна ошибка живёт вечно и обрастает выводами. Фразы «вывод прежний»,
«как я говорил, ДРР такой-то» без свежей проверки запрещены; ПОСЛЕ свежей
проверки наоборот — скажи прямо «проверил заново: вывод тот же, изменилось
только …» и не пересказывай старый разбор.
Если инструмент вернул время сборки данных (built_at/fetched_at) старше
пары часов — оговори возраст цифр одной фразой.
    ...
    try:
        ...
        if light and focus:
            user_msg += ("\nБЫСТРЫЙ РЕЖИМ — вопрос владельца в КОНЦЕ сообщения.\n"
                         ...
                         "инструментов; на уточняющий вопрос по уже разобранной "
                         "теме бери сколько нужно, чтобы копнуть ГЛУБЖЕ (кампании "
                         "→ фразы/ставки/дни), а не пересказать прежнее. "
                         "wb_search не используй. Память читай только если вопрос "
                         "про план/задачи/факты; save_memory — только если "
                         "появилось что сохранить.")
        elif focus:
            user_msg += "\nФокус этой сессии — вопрос владельца в конце сообщения."
        else:
        ...
        try:
            ...
            if _snapshot:
                user_msg += ("\n\n" + _snapshot +
                             "\n(Это сводка — только фон для навигации, НЕ "
                             "источник цифр для ответа и НЕ план ответа: вопрос "
                             "про рекламу — дёрни advertising, про остатки — "
                             "stocks. Если нужных данных нет — dashboard_catalog "
                             "и dashboard_api дают доступ ко всему дашборду.)")
        ...
        # Последний ответ храним дословно (чтобы агент видел, ЧТО уже сказал и
        # что предложил в конце), более старые — с вырезанными числами
        import snapshot as _snap
        _dlg_key = (f"strategist_dialog{('_t' + str(thread)) if thread else ''}"
                    f"{('_c' + chat) if chat else ''}")
        dialog = await asyncio.to_thread(_snap.load, _dlg_key, None) or []
        if dialog and focus:
            lines = [f"- Владелец: {d['q']}\n  Ты ответил: {_strip_numbers(d['a'])}"
                     for d in dialog[-6:-1]]
            last = dialog[-1]
            lines.append(f"- Владелец: {last['q']}\n  Твой ПОСЛЕДНИЙ ответ "
                         f"(дословно): {last['a']}")
            if last.get("offer"):
                lines.append(f"  В конце ты предложил: {last['offer']}")
            recent = "\n".join(lines)
            user_msg += ("\n\nНЕДАВНИЙ ДИАЛОГ. Если сейчас пришла короткая "
                         ...
                         "Если новый вопрос — по теме, которую ты уже разбирал "
                         "ниже, НЕ повторяй тот разбор: дай дельту и копни "
                         "глубже (см. анти-правило). "
                         "ЦИФРЫ из прошлых ответов — только напоминание о теме, "
                         "источником данных они НЕ являются: пересчитай заново. "
        ...
        for _step in range(_MAX_STEPS):
            ...
            msg = await client.messages.create(
                model=_MODEL, max_tokens=1200 if light else 2500,
            ...
            if msg.stop_reason != "tool_use":
                ...
                if report:
                    ...
                    # в память диалога пишем и вопросы, и регулярные отчёты —
                    # иначе после утреннего отчёта агент «не помнил», что уже
                    # рассказал, и заводил ту же шарманку на первый вопрос
                    _offer = ""
                    _rl = [ln for ln in report.strip().splitlines() if ln.strip()]
                    if _rl:
                        _offer = _rl[-1][:200]
                    dialog.append({
                        "q": focus[:300] if focus else f"(регулярный отчёт: {trigger})",
                        "a": report[:1200], "offer": _offer})
                    await asyncio.to_thread(_snap.save, _dlg_key, dialog[-15:])
