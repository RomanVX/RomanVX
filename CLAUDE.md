...
## МПФИТ — WMS нашего фулфилмента (работаем в нём ежедневно)

МПФИТ (app.mpfit.ru) — облачная WMS, в которой живёт фулфилмент Романа в
Чехове (FBS WB/Ozon, приёмка/ячейки/сборка/биллинг клиентов). **Полная
выжимка по платформе — `docs/mpfit.md`** (подключение МП, связка товаров
по баркодам, FBS-цикл, умная лента, КИЗы, биллинг услуг/хранения, ТСД,
тарифы, REST API, карта всей справки help.mpfit.ru). При любом вопросе про
МПФИТ сначала смотреть туда. Топ-грабли: read-only токен WB не даёт
включить управление; включение «Управления остатками» Ozon до внесения
остатков шлёт на Ozon нули; синхронизацию баркодов при связке жать один
раз и только у одного МП; «В архив» у заказа «готов к отгрузке» не снимает
бронь (прогнать Контроль→Отгрузка→Размещение); этикетки ЯМ — только
вручную. МПФИТ также референс для собственной WMS (`docs/wms_spec.md`).
